"""
RS Downloader v10.0.0 - Dailymotion Download Agent
====================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Dailymotion downloader supporting:
...
class DailymotionDownloader(DownloaderBase):
    """Dailymotion download agent using yt-dlp."""

    AGENT_NAME = "dailymotion"
    PLATFORM = "dailymotion"
    SUPPORTED_FORMATS = ["mp4", "webm", "mkv", "mp3", "m4a"]
    SUPPORTED_QUALITIES = ["best", "1080p", "720p", "480p", "360p", "audio"]

    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?dailymotion\.com/video/\w+'),
        re.compile(r'https?://(www\.)?dailymotion\.com/playlist/\w+'),
        re.compile(r'https?://(www\.)?dailymotion\.com/\w+/videos'),
        re.compile(r'https?://dai\.ly/\w+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Dailymotion URL: {task.url}")
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="dailymotion",
        formats=frozenset(DailymotionDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(DailymotionDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "playlists", "channels"]),
        max_concurrent=2,
        priority=DownloadPriority.NORMAL,
        version="10.0.0",
        description="Dailymotion downloader: videos, playlists, channel content.",
    )
    return ("dailymotion", skill)
