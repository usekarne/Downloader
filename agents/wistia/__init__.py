"""
RS Downloader v10.0.0 - Wistia Download Agent
================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Wistia downloader supporting:
- Video downloads via embed/JSON extraction
...
class WistiaDownloader(DownloaderBase):
    """Wistia download agent using yt-dlp and embed/JSON extraction."""
    ...
    _URL_PATTERNS = [
        re.compile(r'https?://[\w-]+\.wistia\.(?:com|net)/medias/[\w]+'),
        re.compile(r'https?://[\w-]+\.wistia\.(?:com|net)/embed/[\w]+'),
        re.compile(r'https?://(www\.)?wistia\.com/\?wvideoid='),
        re.compile(r'wistia_[\w]+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Wistia URL: {task.url}")
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="wistia",
        formats=frozenset(WistiaDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(WistiaDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "embed_extraction", "password_protected"]),
        max_concurrent=1,
        priority=DownloadPriority.LOW,
        version="10.0.0",
        description="Wistia downloader: videos via embed/JSON extraction.",
    )
    return ("wistia", skill)
