"""
RS Downloader v10.0.0 - Periscope Download Agent
===================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Periscope downloader supporting:
- Broadcast replays
- Live broadcasts
"""
...
class PeriscopeDownloader(DownloaderBase):
    """Periscope download agent using yt-dlp for broadcast replays."""

    AGENT_NAME = "periscope"
    PLATFORM = "periscope"
    SUPPORTED_FORMATS = ["mp4", "webm", "mp3", "m4a"]
    SUPPORTED_QUALITIES = ["best", "high", "medium", "low"]

    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?pscp\.tv/\w+/\w+'),
        re.compile(r'https?://(www\.)?periscope\.tv/\w+/\w+'),
        re.compile(r'https?://(www\.)?periscope\.tv/\w+$'),
        re.compile(r'https?://(www\.)?pscp\.tv/w/[\w]+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Periscope URL: {task.url}")
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="periscope",
        formats=frozenset(PeriscopeDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(PeriscopeDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["replays", "live_broadcasts"]),
        max_concurrent=1,
        priority=DownloadPriority.LOW,
        version="10.0.0",
        description="Periscope downloader: broadcast replays and live broadcasts.",
    )
    return ("periscope", skill)
