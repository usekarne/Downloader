"""
RS Downloader v10.0.0 - Odysee Download Agent
================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Odysee (LBRY) downloader supporting:
...
class OdyseeDownloader(DownloaderBase):
    """Odysee/LBRY download agent using yt-dlp."""

    AGENT_NAME = "odysee"
    PLATFORM = "odysee"
    SUPPORTED_FORMATS = ["mp4", "webm", "mkv", "mp3", "m4a", "ogg", "flac"]
    SUPPORTED_QUALITIES = ["best", "1080p", "720p", "480p", "360p", "audio"]

    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?odysee\.com/@[\w.-]+/[\w.-]+'),
        re.compile(r'https?://(www\.)?odysee\.com/[\w.-]+: [\w]+'),
        re.compile(r'https?://(www\.)?odysee\.com/[\w.-]+/[\w.-]+'),
        re.compile(r'https?://odysee\.com/\$/(?:claim|download|embed)/'),
        re.compile(r'https?://lbry\.tv/[\w.@-]+/[\w-]+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Odysee URL: {task.url}")
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="odysee",
        formats=frozenset(OdyseeDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(OdyseeDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "audio", "channels", "lbry"]),
        max_concurrent=2,
        priority=DownloadPriority.NORMAL,
        version="10.0.0",
        description="Odysee/LBRY downloader: videos, audio, channel content.",
    )
    return ("odysee", skill)
