"""
RS Downloader v10.0.0 - BitChute Download Agent
==================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

BitChute downloader supporting:
- Videos (all available qualities)
"""
...
class BitChuteDownloader(DownloaderBase):
    """BitChute download agent using yt-dlp."""
    ...
    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?bitchute\.com/video/\w+'),
        re.compile(r'https?://(www\.)?bitchute\.com/channel/[\w-]+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid BitChute URL: {task.url}")
    ...
    def on_download(self, task: DownloadTask) -> DownloadResult:
        ...
        ytdlp_opts = self._build_ytdlp_opts({
            "format": self._resolve_format(quality),
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="bitchute",
        formats=frozenset(BitChuteDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(BitChuteDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "channels"]),
        max_concurrent=1,
        priority=DownloadPriority.LOW,
        version="10.0.0",
        description="BitChute downloader: video downloads.",
    )
    return ("bitchute", skill)
