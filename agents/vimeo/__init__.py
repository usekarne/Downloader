"""
RS Downloader v10.0.0 - Vimeo Download Agent
================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Vimeo downloader supporting:
- Videos (all qualities including original)
- Channels
- Albums/collections
...
class VimeoDownloader(DownloaderBase):
    """Vimeo download agent using yt-dlp with channel and album support."""

    AGENT_NAME = "vimeo"
    PLATFORM = "vimeo"
    SUPPORTED_FORMATS = ["mp4", "webm", "mkv", "mp3", "m4a", "flv"]
    SUPPORTED_QUALITIES = ["best", "4K", "1080p", "720p", "480p", "360p", "audio"]

    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?vimeo\.com/\d+'),
        re.compile(r'https?://(www\.)?vimeo\.com/channels/[\w-]+'),
        re.compile(r'https?://(www\.)?vimeo\.com/channels/[\w-]+/\d+'),
        re.compile(r'https?://(www\.)?vimeo\.com/album/\d+'),
        re.compile(r'https?://(www\.)?vimeo\.com/groups/[\w-]+'),
        re.compile(r'https?://(www\.)?vimeo\.com/[\w-]+/review/\w+'),
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Vimeo URL: {task.url}")
    ...
    def _resolve_format(quality: str) -> str:
        quality_map = {
            ...
            "360p": "worstvideo+worstaudio/worst",
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="vimeo",
        formats=frozenset(VimeoDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(VimeoDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "channels", "albums", "password_protected", "original_quality"]),
        max_concurrent=2,
        priority=DownloadPriority.NORMAL,
        version="10.0.0",
        description="Vimeo downloader: videos, channels, albums with quality selection.",
    )
    return ("vimeo", skill)
