"""
RS Downloader v10.0.0 - Rumble Download Agent
================================================

Author: RAJSARASWATI JATAV (RS) / T3rmuxk1ng

Rumble downloader supporting:
...
class RumbleDownloader(DownloaderBase):
    """Rumble download agent using yt-dlp."""
    ...
    _URL_PATTERNS = [
        re.compile(r'https?://(www\.)?rumble\.com/\w+-[\w.-]+\.html'),
        re.compile(r'https?://(www\.)?rumble\.com/c/[\w.-]+'),
        re.compile(r'https?://(www\.)?rumble\.com/v\w+'),
        re.compile(r'https?://(www\.)?rumble\.com/user/[\w.-]+'),
    ]
    ...
    def on_prepare(self, task: DownloadTask) -> None:
        if not self.validate_url(task.url):
            raise DownloadError(f"Invalid Rumble URL: {task.url}")
...
def register() -> Tuple[str, AgentSkill]:
    skill = AgentSkill(
        platform="rumble",
        formats=frozenset(RumbleDownloader.SUPPORTED_FORMATS),
        qualities=frozenset(RumbleDownloader.SUPPORTED_QUALITIES),
        features=frozenset(["video", "livestreams", "channels"]),
        max_concurrent=2,
        priority=DownloadPriority.NORMAL,
        version="10.0.0",
        description="Rumble downloader: videos, livestreams, channel content.",
    )
    return ("rumble", skill)
